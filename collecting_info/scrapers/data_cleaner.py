...
def main():
    ...
    df = pd.read_csv(DATA_DIR / 'country_info.csv')

    # Add duck data
    duck = pd.read_csv(DATA_DIR / 'duck_populations.csv', dtype={'Population Count': str, 'Ducks Per Capita (per 1.000)': str})
    duck['name'] = duck['Country/Territory']
    duck.reset_index(inplace=True)
    duck['index'] = duck['index'] + 1
    duck['duck_pop_rank'] = duck['index'].astype(str)
    ...
    # Add continents
    continents = pd.read_csv(DATA_DIR / 'list-of-countries-by-continent-2026.csv')
    continents.rename(columns={'country': 'name'}, inplace=True)
    df = pd.merge(df, continents[['name', 'continent']], on='name', how='left')

    # Final Save
    df.to_csv(DATA_DIR / 'country_info_updated.csv', index=False)
